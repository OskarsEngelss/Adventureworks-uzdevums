...
def extract_transform_combine_product_data_into_dimproduct_and_upload_to_starrocks():
    ...
    def synchronize_postgresql_to_starrocks():
        ...
        # 1. Clean Staging (ensure you have this stg_dim_product_upsert table created)
        ...
        load_staging_sql = f"""
            ...
            SELECT 
                ...
                pc.CategoryName AS Category,  -- Now pulling from your Master Dim Table
            ...
            -- JOINING TO YOUR NEW MASTER DIMENSION
        ...
        # 3. EXPIRE logic (Detect changes in ListPrice, Cost, Category, or Status)
        expire_sql = f"""
            UPDATE DimProduct
            SET IsCurrent = FALSE, 
            ...
            AND (
                ...
                DimProduct.ProductStatus != s.ProductStatus
            );
        """
        starrocks_hook.run(expire_sql)

        # 4. INSERT logic (New products + New versions of changed products)
        insert_sql = f"""
